...
class HomeViewController: EvaluateBaseController {
    // documents
    var documents: [File] = [] {
        didSet {}
    }
        ...
        didSet {
            DispatchQueue.main.async {
                if self.documentsOfPDF.isEmpty {
                    self.tableView.showEmpty(message: String.localize("HomeVC.empty.addtips"))
                } else {
...
// MARK: HomeViewController的私有方法

extension HomeViewController {
    private func initUI() {
//        title = "文档扫描小助手"
        let label = UILabel()
        label.text = String.localize("app.name")
        label.font = MyFont.font(with: .bold, size: 20)
        navigationItem.leftBarButtonItem = UIBarButtonItem(customView: label)
        // right items
        navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "gear")?.withRenderingMode(.alwaysOriginal).withTintColor(.label), style: .plain, target: self, action: #selector(nav2setting))
        ...
        NSLayoutConstraint.activate([
            ...
            addBtn.widthAnchor.constraint(equalToConstant: HomeViewController.addBtnWidth),
            addBtn.heightAnchor.constraint(equalToConstant: HomeViewController.addBtnWidth)
        ])
        // 给addBtn增加点击事件
    ...
    @objc private func fetchDocuments() {
        let list = MyFileManager.shared.scanDirectory2Files(with: MyFileManager.shared.documentPath)
...
extension HomeViewController: PHPickerViewControllerDelegate, VNDocumentCameraViewControllerDelegate {
    // pdf version 代理
    // 扫描成功
    ...
    func pic2Document(images: [UIImage]) {
        ...
        vc.closure = { [weak self] name in
            ...
            DispatchQueue.global().async {
                ...
                let docURL = MyFileManager.shared.getUniqueFileUrl(name: name, type: ".pdf")
                do {
...
extension HomeViewController: FolderCellDelegate, ActionSheetDelegate {
